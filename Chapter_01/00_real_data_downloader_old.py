...
def download_era5_multivar():
    ...
    variables = [
        ...
        "temperature_2m_mean",
        "precipitation_sum",
        "wind_speed_10m_max",        # New name (replaces windspeed_10m_max)
        "wind_gusts_10m_max",        # New name (replaces windgusts_10m_max)
        "relative_humidity_2m_mean",
        # "surface_pressure",        # NOT available as daily in Open-Meteo archive
        "snowfall_sum",
    ...
    # Only keep columns that actually exist (robust to API changes)
    monthly_agg = {k: v for k, v in monthly_agg.items() if k in df.columns}
    monthly = df.resample("ME").agg(monthly_agg).reset_index()
...
def download_admin_boundaries():
    ...
    Uses geodatasets package (geopandas >= 1.0) or falls back to
    Natural Earth direct download if geodatasets is not available.
    """
    ...
    # Method 1: geodatasets (geopandas >= 1.0 recommended approach)
    try:
        import geodatasets
    ...
    # Method 2: direct URL download from Natural Earth
    if world is None:
        ne_url = (
        ...
        try:
            print("       Downloading Natural Earth boundaries from S3...")
            r = requests.get(ne_url, timeout=60)
            r.raise_for_status()
            with zipfile.ZipFile(io.BytesIO(r.content)) as z:
                tmp = os.path.join(OUT_DIR, "ne_tmp")
                z.extractall(tmp)
            shp = [os.path.join(tmp, f) for f in os.listdir(tmp) if f.endswith(".shp")]
            if shp:
                world = gpd.read_file(shp[0])
                shutil.rmtree(tmp)
        except Exception as e:
            print(f"       [WARNING] Natural Earth download failed: {e}")

    # Method 3: legacy gpd.datasets (geopandas < 1.0)
    if world is None:
        try:
    ...
    if world is None:
        print("       [WARNING] Could not load admin boundaries.")
        return None

    # Filter to Chile + Argentina
    name_col = next((c for c in ["NAME", "name", "ADMIN", "admin"] if c in world.columns), None)
    if name_col:
        chile_arg = world[world[name_col].isin(["Chile", "Argentina"])]
    else:
        # Fallback: clip to South America extent
        chile_arg = world.cx[-82:-52, -56:-15]

    chile_arg.to_file(out_path, driver="GPKG")
    print(f"       [SUCCESS] Admin boundaries saved: {out_path}  ({len(chile_arg)} features)")
    return chile_arg
...
def plot_data_summary(df_era5_daily, df_stations):
    ...
    # Panel 4: Wind
    ax = axes[1, 0]
    wind_col = next((c for c in ["wind_speed_10m_max", "windspeed_10m_max"] if c in df.columns), None)
    if wind_col:
        annual_wind = df[wind_col].resample("ME").mean()

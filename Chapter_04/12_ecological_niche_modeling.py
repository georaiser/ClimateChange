...
Academic Objective:
Species Distribution Modeling (SDM), also known as Ecological Niche Modeling,
predicts the geographic distribution of a species based on environmental variables.

In this script, we replicate the "MaxEnt" (Maximum Entropy) or Suitability Modeling
workflow commonly done in ArcGIS Pro. We use Python's Machine Learning library,
Scikit-Learn, to train a Random Forest classifier. 

We will simulate the habitat preference of the endangered Patagonian Huemul Deer,
which prefers low elevations, gentle slopes, and healthy vegetation (high NDVI).
The model will learn these patterns from synthetic presence/absence points and 
predict a continuous "Probability of Occurrence" map across Torres del Paine.
...
def train_and_predict_niche(dem, slope, ndvi, profile):
    ...
    print(f"       Model Accuracy on training data: {rf.score(X_train, y_train)*100:.2f}%")
    print("       Feature Importances (Elevation, Slope, NDVI):", np.round(rf.feature_importances_, 3))
    
    print("\n[INFO] Predicting Habitat Probability across Torres del Paine...")
    # Predict Probability of Occurrence (Class 1) across the entire grid
    # Some pixels might have NaNs (ocean/nodata), so we fill them with 0 safely
    X_full = np.nan_to_num(X_full, 0)
    probabilities = rf.predict_proba(X_full)[:, 1] # Get probability of presence (1)
    ...
    print("\n[INFO] Generating Niche Map Plot...")
    fig, axs = plt.subplots(1, 2, figsize=(16, 8))
    
    # Show the environment composite (RGB: NDVI, Slope, Elevation scaled)
    # Just visualizing the DEM for context
    im1 = axs[0].imshow(dem, cmap='terrain')
    axs[0].set_title("Environmental Predictors (DEM)")
    fig.colorbar(im1, ax=axs[0], shrink=0.7)
    axs[0].axis('off')
    
    # Show the ML Prediction
    im2 = axs[1].imshow(niche_map, cmap='YlGn', vmin=0, vmax=1)
    axs[1].set_title("Predicted Ecological Niche\n(Huemul Habitat Suitability)")
    fig.colorbar(im2, ax=axs[1], shrink=0.7, label='Probability of Occurrence')
    axs[1].axis('off')
    
    plt.tight_layout()
    plot_path = os.path.join(OUT_DIR, "ecological_niche_model.png")
    plt.savefig(plot_path, dpi=300)
    print(f"       [SUCCESS] Niche map saved to: {plot_path}")
...
def main():
    ...
    train_and_predict_niche(dem, slope, ndvi, profile)
